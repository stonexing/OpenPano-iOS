...
#define CFG(x) \
    x = Config.get(#x)
    const char* config_file = [[[NSBundle mainBundle] pathForResource:@"config" ofType:@"cfg"] cStringUsingEncoding:NSUTF8StringEncoding];
...
void panomain(int argc, char* argv[]) {
    if (argc <= 2)
...
@implementation OpenPano

+ (OpenPano*)instance{
    static OpenPano* panoinstance = nil;
    if (panoinstance == nil){
        panoinstance = [[OpenPano alloc] init];
...
- (NSString *)stitchImageFiles:(NSArray *)pathFiles{
    ...
    for (int i=0; i < fileCount; i++){
        NSString * filepathString = [pathFiles objectAtIndex:i];
        const char *filePathChars = [filepathString cStringUsingEncoding:NSUTF8StringEncoding];
        imgs.emplace_back(filePathChars);
    }
